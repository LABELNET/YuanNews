...
<head>
  ...
  <script>
    ...
    var likeDeleteUrl=baseUrl+"managerDeleteComment.action";
  ...
  <div class="rt_content" style="margin-top: 10px;">
    ...
    <section>
      ...
      <table class="table center" style="margin-top:10px;">
        ...
        <c:if test="${empty commentPos}">
           <tr>
             <td colspan="3">没有更多数据了</td>
           </tr>
        </c:if>
        <c:if test="${!empty commentPos}">
          <c:forEach items="${commentPos}" var="commentPo">
            <tr>
              <td>${commentPo.id}</td>
              <td><img style="width:30px;" src="${commentPo.userVo.head}"/></td>
              <td>${commentPo.userVo.nick}</td>
              <td>${commentPo.newsVo.title}</td>
              <td>${commentPo.content}</td>
              <td>${commentPo.status==0?"好的评论":"评论已删除"}</td>
              <td>
                <a class="inner_btn" onclick="showDialog(${commentPo.id})">删除信息</a>
              </td>
            </tr>
          </c:forEach>
        </c:if>
